...
Module Groups
    ...
    Sub permission(type As String, username As String, mode As String, Optional quiet As Boolean = False)
        ...
        Catch ex As Exception
            If DebugMode = True Then
                Wln("You have either found a bug, or the permission you tried to add or remove is already done, or other error." + NewLine + 
                    "Error {0}: {1}" + NewLine + "{2}", "neutralText", Err.Number, Err.Description, ex.StackTrace)
    ...
    Sub permissionEditForNewUser(oldName As String, username As String)
        ...
        Catch ex As Exception
            If DebugMode = True Then
                Wln("You have either found a bug, or the permission you tried to edit for a new user has failed." + NewLine + 
                    "Error {0}: {1}" + NewLine + "{2}", "neutralText", Err.Number, Err.Description, ex.StackTrace)
                Wdbg(ex.StackTrace, True)
            Else
                Wln("You have either found a bug, or the permission you tried to edit for a new user has failed." + NewLine + 
